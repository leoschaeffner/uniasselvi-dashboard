...
Commit de referência: `f42892e` (PATCH 174, backend de Turnover de Tutores)
+ **PATCH 175** (frontend: cards "Engajamento" e "Turnover de Tutores" em
`template_gestor.html`, consumindo `DB.kpis` (raiz) e `DB.turnover`).
+ **PATCH 176** (backend: `dados['gerenciamento_engajamento']` — o card
"Engajamento" do PATCH 175 mostrava engajamento de TUTOR (envio de
portfólio), pedido errado; o certo é engajamento de ALUNO, % de ofertas de
...
`template_dashboard.html`, só resumidos:
  ...
  2. **Card "Gerenciamento (por Tutor)"** (`renderGerenciamentoTutor`) =
     ...
     ordem` (só ofertas COM `ordem`, cadastro-only não entra aqui) e `por
     curso` — ver revisão abaixo.
  - **Revisão do mesmo PATCH 178** (mesmo dia, confirmado pelo Leo): "por
    curso" **NÃO** usa a taxonomia de ~27 cursos de
    `gerenciamento_engajamento.por_curso`/`CURSOS_NOMES` (tentativa inicial)
    — usa o agrupamento AMPLO de 5 grupos que o próprio dashboard já usa no
    seletor "Gerenciamento → Detalhe de Ofertas" (`GRUPOS_GER`/
    `_grupoBaseDaCategoria`/`_rowMatchesGrupo`/`_tutorMatchesGrupo`), porque
    é esse o agrupamento que o Leo usa pra conferir esse card manualmente
    (ex: "Biomedicina/Farmácia/Estética, Ordem 3 ≈ 54% gerenciaram / 36%
    obras / 31% treinamento" — na taxonomia de 27 cursos essa combinação
    nem existe, Biomedicina/Farmácia/Estética são 3 linhas separadas lá).
    Mirror literal em `template_gestor.html` (sufixo "Gestor" nas funções só
    pra não colidir de nome com as do dashboard, mesma lógica):
    `GRUPOS_GER_GESTOR` (5 grupos, sem subfiltro — aqui sempre "todo o
    grupo"), `_grupoBaseDaCategoriaGestor`, `_rowMatchesGrupoGestor` (ofertas,
    usa `o.categoria`/`o.subcurso`), `_tutorMatchesGrupoGestor` (cadastro-only,
    usa `t.c`/`_subcursoDeCodigoGestor(t.cursos)`). Mesmo tratamento visual
    do card antigo (top piores + tabela buscável recolhível), só que agora
    com 5 linhas em vez de ~27 — o toggle "ver todos" fica meio redundante
    com só 5 grupos (mostra a mesma coisa que já está em destaque), mas não
    quebra nada; não removido porque não vale a pena um layout separado só
    pra 5 itens. Revalidado contra o `DB` real decifrado (jsdom, mesma rodada
    2026-09-14): grupo "Biomedicina, Farmácia e Estética/Cosmética" = 94
    tutores, 48,9% gerenciaram (qualquer ordem) / 46,8% especificamente na
    Ordem 3 / 23,4% em treinamento / **30,9% em obras** (Leo: 31%, quase
    exato — confirma que o agrupamento em si está certo). Gerenciaram e
    treinamento ficaram abaixo do exemplo do Leo (54%/36%) — diferença
    plausível por deriva de data (`_diasDesdeAdmissaoGestor` calcula "dias
    desde admissão" a partir de `new Date()`, então muda todo dia; o exemplo
    do Leo é de uma conferência anterior) e porque o card mostra "gerenciou
    em QUALQUER ordem" como padrão do recorte "por curso" (a % específica da
    Ordem 3 sozinha, 46,8%, está mais perto ainda). Testado via jsdom
    (`tests/lib/jsdom_portal.js` + `saida/gestor.html` decifrado, script
    ad-hoc) — 0 erros. Não mexeu em
    `template_dashboard.html`/`template_coordenadores.html` — só reaproveitou
    a LÓGICA de lá. `dados['gerenciamento_engajamento']` (PATCH 176) continua
    no backend, só não é mais consumido no `template_gestor.html`.)
...
retorno: `p1, p2, tmpl, p3, p3b, p4, p5, p6, p7, p8, p9` (PATCH 157 acrescentou
`p7`; PATCH 167 acrescentou `p8`; PATCH 172 acrescentou `p9`).
...
| `p9` | `VISTORIA_LAB.xlsx` | `URL_VISTORIA` | Não | **Vistoria de Laboratório** — aba `Registro` (aba oculta `Listas` só alimenta os dropdowns): Data, Multiplicador, Polo, Categoria de Laboratório, Status (`Apto`/`Não Apto`), Motivo, Empresa Responsável, Observações. Alimenta `dados['laboratorios']['vistorias']` — overlay vivo com data em cima do `labs_pendencias.json` estático (não o substitui ainda). Mesmo padrão de multiplicador/polo-combinado/coalescing de colunas do `p8`/Ocorrências. Sem PII de terceiro. PATCH 172. |
...
| Função | Linha ~ | O que faz |
|---|---|---|
| `_calcular_turnover(tutores, tutores_desligados)` | ~2333 | **PATCH 174.** Entrada/saída de tutores por período (semana/mês/semestre ativo) + quebra por curso → `dados['turnover']`. Datas por `_ocor_parse_data` (BR/US tolerante). Ver histórico (§13, PATCH 174). |
...
| `processar_ocorrencias(p8)` | ~3043 | **PATCH 167/169/171.** Lê `OCORRENCIAS.xlsx` (aba `Registro`, ignora `Listas`). Ignora linhas totalmente vazias. Canoniza Multiplicador/Tipo/Gravidade/Status contra as listas fixas da planilha (`_OCOR_MULT_MAP` etc., tolerante a acento/maiúscula via `_vrh_norm`). Status em branco → `'Aberta'` (PATCH 169, Forms não preenche essa coluna). Polo vem como dropdown combinado `"Nome do Tutor — Polo/UF"`, separado por `_ocor_parse_polo_tutor` (PATCH 171; sem separador, é o próprio nome do polo). Campos que se repetem por trilha de branching do Forms (Polo/Curso/Categoria/Tipo/Gravidade/Descrição) são coalescidos por `_col_all`/`_val_multi` locais (PATCH 171 — Status/Responsável/Data de Resolução ficam de fora, são preenchidos manualmente depois, não vêm do Forms). Datas por `_ocor_parse_data` (mesma lógica BR/US de `_interpretar_data_contratacao`, nunca assume formato fixo). Retorna `{registros, kpis, por_multiplicador, por_tipo, por_gravidade, por_polo}` — sem PII de terceiro (é registro de operação, não de aluno/candidato). Alimenta o "Painel do Gestor". |
| `processar_vistoria(p9)` | ~3246 | **PATCH 172.** Réplica do mesmo padrão de `processar_ocorrencias` pro domínio de Vistoria de Laboratório. Lê `VISTORIA_LAB.xlsx` (aba `Registro`, ignora `Listas`). Reaproveita `_OCOR_MULT_MAP`/`_OCOR_MULTIPLICADORES` (Multiplicador) e `_ocor_parse_polo_tutor` (aliasado como `_vist_parse_polo_tutor`, Polo combinado tutor+polo) direto de Ocorrências — não duplica. Categoria de Laboratório canonizada por `_VIST_CAT_MAP`/`_VIST_CATEGORIAS` (8 opções, próprias deste patch). Status (`Apto`/`Não Apto`, `_VIST_STATUS_MAP`) **não leva default quando vazio** (ao contrário de Ocorrências — aqui é o próprio dado da vistoria, deveria sempre vir preenchido; se vier vazio, fica fora das contagens sem quebrar o pipeline). Mesmo coalescing de colunas duplicadas por branching de Forms que o PATCH 171 (`_col_all`/`_val_multi` locais, aplicados a Polo/Categoria/Status/Motivo/Empresa/Observações). Retorna `{registros, kpis: {total, aptos, nao_aptos, pct_apto (fração 0..1), ultima_vistoria}, por_categoria, polos_nao_aptos_recentes}` — sem PII de terceiro. Alimenta `dados['laboratorios']['vistorias']` (overlay novo, não mexe em `labs_pendencias.json`/`laboratorios_data.json`). |
...
> `renderEngajamento`, `renderTurnover` — PATCH 175, `renderOcorrencias`,
> `renderLaboratorios`, `renderVistoria` — PATCH 173, `_iniciarGestor`). Só
> entra na "regra de replicar" quando a mudança é em Ocorrências/Laboratórios/
> Vistoria. Engajamento e Turnover são exclusivos do Gestor (não existem como
> card próprio no dashboard/coordenadores — lá a informação equivalente já
> aparece de outra forma: dado de engajamento de aluno (`DB.gerenciamento_engajamento`,
> PATCH 176) é o mesmo `pct_gerenciado` que o dashboard completo já mostra em
> "ger-visao"/"ger-detalhe", só resumido em 3 recortes pro Gestor; Turnover
> não tem equivalente ainda). Ver tabela de arquivos (§2) e
> histórico (§13, PATCH 168/173/175).
...
2. **Baixar planilhas** — script Python inline. `sp_download_url()` converte link
   ...
   está pronto no workflow), `URL_VISTORIA` (PATCH 172 — mesma situação,
   download pronto, secret ainda não configurado). Só CONTROLE e PORTFOLIO são
   obrigatórios.
...
| `dados['gerenciamento_engajamento']` | `processar.py` (~L5670, logo após `dados['tem_gerenciamento']=True`) | **PATCH 176.** Engajamento de ALUNO (não de tutor): `{'acumulado': {total_ofertas, gerenciadas, pct_gerenciado}, 'por_curso': [...], 'por_ordem': [...]}`, calculado a partir de TODAS as ofertas de TODOS os semestres de `ger_por_semestre` somadas (não só o semestre ativo). Reaproveita a condição `gerenciado` já usada nos `ger_kpis` de cada semestre. `por_curso` usa o mesmo `CURSOS_NOMES`/`split('|')[0]`/"Sem curso" de `_calcular_turnover`; ordenado por `pct_gerenciado` asc (pior primeiro). `por_ordem` ignora ofertas sem ordem (placeholder injetado — PATCH 32); ordenado Ordem 1→5. |
...
| # | Resumo |
|---|---|
| 176 | [py] **`dados['gerenciamento_engajamento']` — engajamento de ALUNO (não de tutor), pro card "Engajamento" do Painel do Gestor.** O PATCH 175 tinha montado esse card em cima de `DB.kpis` (envio de portfólio = engajamento de TUTOR) — pedido errado, corrigido pelo Leo: o card deveria mostrar **% de gerenciamento** (ofertas de prática geridas), quebrado em `acumulado` (TODAS as ofertas de TODOS os semestres somadas — não só o semestre ativo), `por_curso` (`CURSOS_NOMES`/`split('|')[0]`/"Sem curso", mesmo padrão de `_calcular_turnover`/PATCH 174, ordenado por `pct_gerenciado` asc — pior primeiro chama mais atenção) e `por_ordem` (Ordem 1→5, ignora ofertas sem ordem = placeholder do PATCH 32). Reaproveita a MESMA lista `ger_ofertas` e a MESMA condição booleana `gerenciado` (tem tutor E tem data de gerenciamento, PATCH 22) já usadas nos 4 blocos `ger_kpis` existentes — não recalcula do zero. Testado isolado (dado sintético, 2 semestres fake, 7 ofertas, resultado conhecido por asserts) antes de aplicar; depois validado contra a planilha real (`python processar.py --sem-browser`): acumulado 23.8% (7667/32244), 27 cursos, 5 ordens. Só backend — o card em si (`renderEngajamento`, `template_gestor.html`, PATCH 175) ainda não foi atualizado pro novo formato, fica pro `frontend-vinci`. |
| 175 | [frontend, gestor] **Cards "Engajamento" e "Turnover de Tutores" no `template_gestor.html`.** "Engajamento": resumo executivo de 4 números lidos direto de `DB.kpis` (raiz, os mesmos campos que já alimentam o card "Visão Geral" do dashboard principal — `total`, `enviaram`, `urgentes`, `atrasados`, `pendentes`; nada recalculado) — % enviaram (`enviaram/total`), urgentes, atrasados, pendentes; sem tabela por Ordem/semestre (isso já existe no dashboard completo). Mesmo espírito de simplificação do PATCH 166 (Vagas RH). "Turnover de Tutores": consome `DB.turnover` (PATCH 174) — 3 blocos (Semana/Mês/Semestre), cada um com contratados (verde, ✓) e demitidos (vermelho, ✗) lado a lado + saldo com seta (▲/▼/—, `contratados-demitidos`), mais uma tabela pequena com os 3 cursos de `por_curso` com mais movimento no semestre (soma `contratados_semestre+demitidos_semestre`, filtra os com movimento>0), reaproveitando `.rank-item`. Ambos os cards têm estado vazio (`turnover-empty`) por segurança mesmo `DB.turnover` sendo fonte não-opcional (roda incondicionalmente desde o PATCH 174). Funções novas: `renderEngajamento`, `renderTurnover`, `_turnoverSaldoHtml`; chamadas em `_iniciarGestor()`, logo após `renderGestorKPIs()`. Não toca `template_dashboard.html`/`template_coordenadores.html` — ambos os cards são exclusivos do Painel do Gestor (Engajamento é uma versão resumida do que o dashboard já mostra em detalhe; Turnover não tem equivalente nos outros dois portais ainda). Testado com jsdom (`saida/gestor.html` gerado local com `processar.py --sem-browser`, decifrado com `SENHA_GESTOR` via Web Crypto do Node + `Object.defineProperty` pra injetar `webcrypto` no `window` do jsdom — a atribuição direta `window.crypto = webcrypto` falha silenciosamente porque o `crypto` do jsdom é uma property não-writable): números batem com o log do pipeline (semana 14/2, mês 15/6, semestre 44/18; Engajamento 356 tutores, 55% enviaram, 190 urgentes, 0 atrasados, 161 pendentes) e os 2 cards renderizam com `display` correto (conteúdo visível, estado vazio oculto). |
| 174 | [py] **`dados['turnover']` — entrada/saída de tutores por período, pro Painel do Gestor.** `_calcular_turnover(tutores, tutores_desligados)` (`processar.py`, perto de `_carregar_laboratorios`), chamada no `__main__` logo após `tem_lotacao` (roda sempre, não depende de nenhum secret opcional). `{'semana', 'mes', 'semestre', 'por_curso'}` — `semana`/`mes`/`semestre` = `{contratados, demitidos, inicio_periodo, fim_periodo}` (datas `DD/MM/AAAA`); `por_curso` = lista `{curso, contratados_semana, demitidos_semana, contratados_mes, demitidos_mes, contratados_semestre, demitidos_semestre}`. Contratados = `tutor['inicio']` (`'YYYY-MM-DD'`) dentro do período; Demitidos = `tutores_desligados[]['data_desligamento']` dentro do período — **ambos passam por `_ocor_parse_data`** (mesmo parser BR/US tolerante de `_interpretar_data_contratacao`/Ocorrências/Vistoria — nunca assume formato fixo), inclusive a data já formatada de `tutores_desligados` (PATCH 106 só formata o que o Excel deu, não resolve a mesma ambiguidade BR/US do PATCH 155/156 — confirmado com a planilha real: 2 demissões "08/12/2026" só fazem sentido como 12/08/2026, senão ficam no futuro). `tutores_desligados[]` ganhou o campo novo `'cursos'` (mesma coluna/formato de `tutores[].cursos`, antes só tinha `'c'`/categoria ampla) pra dar pro `por_curso` agrupar do mesmo jeito nos dois lados (`CURSOS_NOMES`, split por `'|'`, mesmo padrão de `processar():~1677` e `gerar_onboarding_atualizado():~3550` — não inventa taxonomia nova). `semana` = últimos 7 dias corridos; `mes` = mês corrente; `semestre` = min(início)..max(fim) das 5 Ordens do `SEMESTRE_ATUAL` (não existe campo separado de "data do semestre" — as Ordens já são o calendário). Testado isolado (dado sintético com asserts) e depois contra a planilha real (`python processar.py --sem-browser`): semana 14 contratados/2 demitidos, mês 15/6, semestre (2026/2) 44/18 — conferido manualmente contra o CONTROLE (2 demissões reais em 07/09/2026 batem com o bucket "semana"). "Simplificar engajamento" (pedido junto) é só frontend (`template_gestor.html`) — não existe cálculo de "engajamento" duplicado no backend pra essa tela hoje.
| 173 | [frontend, gestor] **Card "Vistoria de Laboratório" no `template_gestor.html`**, consumindo o overlay `DB.laboratorios.vistorias` do PATCH 172. Segue o mesmo desenho visual/estados do card "Ocorrências por Multiplicador" (PATCH 168): se `DB.laboratorios.vistorias` não existir ou `kpis.total===0` (situação real hoje, secret `URL_VISTORIA` ainda não configurado), mostra estado vazio explícito (`vist-empty`, mesmo texto/tom do de Ocorrências); com dado presente, mostra `% Apto`/`Aptos·Não Aptos`/`Última Vistoria` (via `kpis`, `pct_apto` é fração 0..1 → multiplicado por 100 só na exibição), barras por categoria (`por_categoria`, cor verde/amarela/vermelha por faixa de `pct_apto`, reaproveitando o padrão `.rank-item`/`.rank-bar` já usado em Ocorrências) e uma tabela buscável de `polos_nao_aptos_recentes` (mesmo padrão de `filterLabPend`, aqui `filterVistNaoApto`). Funções novas: `_vistAptoCor`, `renderVistoria`, `filterVistNaoApto`; chamada em `_iniciarGestor()`. Não mexe nos cards de Ocorrências nem em Laboratórios — Pendências (estático, `labs_pendencias.json`) além de inserir o card novo logo abaixo. Não toca `template_dashboard.html`/`template_coordenadores.html` — a fonte é exclusiva do Painel do Gestor por ora. Testado com jsdom (`node --eval` do script inline extraído do template) nos dois cenários: `DB.laboratorios.vistorias` ausente (estado real de produção hoje) e populado com 4 registros sintéticos (2 aptos/2 não aptos, incluindo motivo/empresa) gerados a partir de `planilhas/VISTORIA_LAB_template.xlsx` copiado pra `VISTORIA_LAB.xlsx`, processado local via `processar.py --sem-browser`, decifrado com `SENHA_GESTOR` e removido depois do teste. |
| 172 | [py] **Nona fonte de dado: Vistoria de Laboratório (`p9`/`URL_VISTORIA`, opcional) — overlay vivo sobre `labs_pendencias.json`.** Mesmo padrão exato do `p8`/Ocorrências (PATCH 167/169/171): `VISTORIA_LAB.xlsx` → `processar_vistoria(p9)`, resolvida em `verificar_e_localizar()` e ligada no `__main__` dentro de `try/except` tolerante — sem o secret, `dados['laboratorios']['vistorias']` nem existe; `labs_pendencias.json`/`laboratorios_data.json` continuam exatamente como estão (esta fonte não os substitui ainda). Schema (aba `Registro`, aba oculta `Listas` só alimenta dropdown): `Data | Multiplicador | Polo | Categoria de Laboratório | Status | Motivo | Empresa Responsável | Observações`. `Multiplicador` reaproveita `_OCOR_MULTIPLICADORES`/`_OCOR_MULT_MAP` de Ocorrências (não duplica); `Categoria de Laboratório` ganha `_VIST_CATEGORIAS` (8 opções, mesma lista usada em Ocorrências); `Status` só tem 2 valores (`Apto`/`Não Apto`) e **não leva default quando vazio** (diferente de Ocorrências: aqui é o próprio dado que o multiplicador está reportando, deveria sempre vir preenchido pelo Forms — se vier vazio mesmo assim, o registro entra na lista mas fica fora das contagens por status, sem quebrar o pipeline). `Polo` usa o mesmo dropdown combinado `"Nome do Tutor — Polo/UF"` do PATCH 171 (`_vist_parse_polo_tutor = _ocor_parse_polo_tutor`, reaproveitado direto — é lógica de texto genérica, não específica de Ocorrências) e o mesmo coalescing de colunas duplicadas por branching de Forms (`_col_all`/`_val_multi` locais, réplica dos de `processar_ocorrencias`, aplicados a Polo/Categoria/Status/Motivo/Empresa/Observações — não a Data/Multiplicador). Retorna `{registros, kpis: {total, aptos, nao_aptos, pct_apto (fração 0..1, não *100), ultima_vistoria}, por_categoria, polos_nao_aptos_recentes (só Não Apto, top 30 por data desc)}`. Sem PII de terceiro. Testado com dado sintético (planilha vazia → `total=0` sem quebrar; linha com Status em branco → fora das contagens; polo em formato combinado tutor+polo; 3 cópias da mesma pergunta simulando branching). Template local `planilhas/VISTORIA_LAB_template.xlsx` (gitignored, gerado com `openpyxl`, mesmo estilo visual do `OCORRENCIAS_template.xlsx`: aba `Registro` com cabeçalho teal + aba oculta `Listas` com `DataValidation`, Polo reaproveitando os 1303 polos de `contatos_por_polo.json`). |
...
| 169 | [py] **Status em branco em Ocorrências vira `'Aberta'`** — Microsoft Forms só escreve as colunas de entrada (até Descrição); Status/Responsável/Data de Resolução ficam em branco até alguém tratar o caso. Sem esse default em `processar_ocorrencias`, um registro vindo do Forms não entrava em nenhuma contagem (`kpis.abertas`/`em_tratativa`/`resolvidas` todas ficavam sem ele). |
